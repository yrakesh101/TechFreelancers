...
import com.example.techfreelancers.adapter.PostedAdapter;
...
public class PostedActivity extends AppCompatActivity {
    ...
    ActivityPostedBinding postedBinding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        postedBinding = ActivityPostedBinding.inflate(getLayoutInflater());
        setContentView(postedBinding.getRoot());

        recyclerView = postedBinding.mostVotedRecyclerView;
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        setSupportActionBar(postedBinding.toolbar);
        ActionBar actionBar = getSupportActionBar();
        if (actionBar != null) {
            actionBar.setDisplayHomeAsUpEnabled(true);
            actionBar.setDisplayShowHomeEnabled(true);
            actionBar.setDisplayShowTitleEnabled(false);
            Drawable navIcon = postedBinding.toolbar.getNavigationIcon();
            if (navIcon != null) {
    ...
    private void fetchSearchProjects() {
        ...
        Call<ResponseModel<List<TechProject>>> call = retrofit.create(ProjectApi.class).queryMyPostedProject();
            ...
            public void onResponse(Call<ResponseModel<List<TechProject>>> call, Response<ResponseModel<List<TechProject>>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (responseModel.getSuccess() && responseModel.getStatus() == 200) {
                        ...
                        adapter = new PostedAdapter(projects);
